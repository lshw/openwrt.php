...
if(empty($host['id'])) goback("没有找到站点");
}
switch($_GET['field']) {
case 'host':
if($_GET['id']=='0') {
$host=$db->fetch_one_assoc("select * from hosts where host='$_GET[host]'");
if(!empty($host['id'])) goback("已经存在");
$db->query("insert into hosts (host) values('$_GET[host]');");
}else {
